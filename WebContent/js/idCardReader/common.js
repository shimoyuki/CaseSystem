...
{
	$("#birthday").val(result.Certificate.Birthday.replace(/\./g,"-").substr(0,10));//生日
	$("#IDCode").val(result.Certificate.IDNumber);//身份证号
	$("#issuingAuthority").val(result.Certificate.IDIssued);//签发机关
	$("#issuedValidDate").val(result.Certificate.IssuedData+"-"+result.Certificate.ValidDate);//有效时间
	
	imgData =result.Certificate.Base64Photo;
	$("#id_img_pers").attr("src","data:image/jpg;base64,"+imgData);
	$("#IDPicture").val(imgData);
	$("#personPhoto").val("");
	
	$("#name").val(result.Certificate.Name);//姓名
	$("#sex").val(result.Certificate.Sex);//性别
	$("#nation").val(result.Certificate.Nation);//民族
	$("#address").val(result.Certificate.Address);//住址
}
